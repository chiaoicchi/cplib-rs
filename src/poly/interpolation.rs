...
/// The value at `c` of the polynomial of degree less than `n` taking the values `y` on `[0, n)`.
///
/// # Definition
/// For `n = y.len()`, let `f` in `R[x]` be the unique polynomial of degree less than `n` with
/// `f(i) = y(i)` for `i` in `[0, n)`. Returns
/// `f(c) = Σ_i y(i) Π_{j!=i} (c - j) / Π_{j!=i} (i - j)`. For `n = 0`, `f` is zero.
///
/// # Contract
/// `(n - 1)!` is invertible in `R`, that is the characteristic is `0` or greater than `n - 1`.
/// Under it `f` exists and is unique, as the Vandermonde determinant `Π_{k<n} k!` is invertible.
///
/// # Complexity
/// - Time: O(n)
/// - Space: O(n)
pub fn lagrange_interpolate_iota<R: Field>(ring: &R, y: &[R::Value], c: &R::Value) -> R::Value {
    let n = y.len();
    if n == 0 {
        return ring.zero();
    }
    ...
    for i in 0..n {
        ...
        let term = ring.mul(&ring.mul(&y[i], &numer), &denom);
        let s = (n - 1 - i) & 1;
...
/// # Definition
/// For `n = y.len()`, let `f` in `R[x]` be the unique polynomial of degree less than `n` with
/// `f(i) = y(i)` for `i` in `[0, n)`. Returns `f(c + k)` for `k` in `[0, m)`. For `m = 1` it is
/// [`lagrange_interpolate_iota`].
///
/// # Contract
/// `(n + m - 2)!` is invertible in `R`.
///
/// # Complexity
/// - Time: O((n + m) log (n + m))
/// - Space: O(n + m)
///
/// # Panics
/// Panics if `R` has no primitive root of unity of the order [`poly_convolve`] needs for length `n`
/// and `n + m - 1`.
pub fn shift_of_sampling_points<R: RootOfUnity<Value: PartialEq>>(
    ring: &R,
    y: &[R::Value],
    c: &R::Value,
    m: usize,
    ...
    for i in 0..n {
        let t = ring.mul(&y[i], &ring.mul(&inv_fact[i], &inv_fact[n - 1 - i]));
        a.push(if (n - 1 - i) & 1 == 1 {
            ring.neg(&t)
        } else {
            t
