...
///
/// # Definition
/// `(Zf)(S) = Σ_{T⊆S} f(T)`, the zeta transform of the boolean lattice on `[n]`.
///
/// # Complexity
/// - Time: O(2^n n)
/// - Space: O(1)
...
///
/// # Definition
/// `(Z^{-1}f)(S) = Σ_{T⊆S} (-1)^{|S|-|T|} f(T)`, by Mobius inversion on the boolean lattice, whose
/// Mobius function is `μ(T, S) = (-1)^{|S|-|T|}`.
///
/// # Complexity
/// - Time: O(2^n n)
/// - Space: O(1)
...
/// The union convolution of `f` and `g`.
///
/// # Definition
/// `(fg)(S) = Σ_{T∪U=S} f(T) g(U)`, the product of the monoid algebra of the union semilattice on
/// `[n]`. Since `T⊆S` and `U⊆S` iff `T∪U⊆S`, [`subset_zeta`] carries it to the pointwise product:
/// `Z(fg) = Z(f) Z(g)`.
///
/// # Complexity
/// - Time: O(2^n n)
